...
with open('C:/watsonx/watsonx-indianinsurance-master/app/src/assets/css/app.css')as f:
...
st.markdown('<h1 style="text-align: center; font-size:65px; color:black; margin:10px;"> 🧑‍💼🚘Digital Claim Agent </h1>', unsafe_allow_html=True)
st.markdown("<h4 style='text-align: center; color: black;'><i>AI-driven insurance claim system powered by watsonx </i></h4>", unsafe_allow_html=True)
...
my_logo = add_logo(logo_path="app/src/assets/images/slic.png", width=200, height=200)
...
with st.expander("Why this application?"):
     st.write("""
         This page aims at summarizing complex Auto insurance claim descriptions, Extract claim entities & suggest next steps from the data scource using watsonx.ai to reduce claim turn around time, reduce manual intervention, streamline and revolutionize the insurance claim industry .
     """) 
...
# +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    with cols[0]:
        if st.session_state.next_steps is not None:
            st.markdown("<h4>Suggested steps for the claims officer</h4>", unsafe_allow_html=True)
    ...
    with cols[1]:
        if st.session_state.entities is not None:  
            ...
            st.markdown("<h4>Entities extracted from the claims</h4>",unsafe_allow_html=True)
